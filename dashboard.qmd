...
teacher_data <- reactive({
  req(input$teacher_id)
  # Valider que l'ID n'est pas nul
  
  validate(need(nchar(input$teacher_id) > 0, ""))
  
  # Valider que l'ID existe dans les données
  validate(need(
    any(data$person_id_secure == input$teacher_id, na.rm = TRUE),
    ""
  ))
...
observe({
  if(length(input$research_scales) == 0) {
    updateSelectInput(session, "research_scales", 
                     selected = unique(data$scale_label)[1])
  }
})

observe({
  if(length(input$research_groups) == 0) {
    updateSelectInput(session, "research_groups", 
                     selected = unique(data$group_id)[1])
  }
})

observe({
  if(length(input$research_status) == 0) {
    updateSelectInput(session, "research_status", 
                     selected = unique(data$status)[1])
  }
})

observe({
  if(length(input$research_gender) == 0) {
    updateSelectInput(session, "research_gender", 
                     selected = unique(data$gender)[1])
  }
})

observe({
  if(length(input$research_age) == 0) {
    updateSelectInput(session, "research_age", 
                     selected = unique(data$age)[1])
  }
})
...
output$research_plot <- renderPlotly({
   data_filtered <- filtered_research_data()
  
  if (is.null(input$researcher_id) || 
     nchar(input$researcher_id) == 0 || 
     !any(data$researcher_id == input$researcher_id, na.rm = TRUE) ||
     nrow(data_filtered) == 0) return(NULL)
