...
```{r}
#| content: "valuebox"
#| title: "Nombre total d'observations"
...
::: {.card title="En quelques mots"}
Description de la fonction et du but de la plateforme de suivi du développement professionnel.
:::
...
scales_list <- data |> 
  distinct(scale, scale_label, score_type_label, scale_description, scale_reference) |> 
  arrange()

unique_scales <- unique(scales_list[c("scale", "scale_label", "scale_description", "scale_reference")])

# Pour chaque échelle, générer le contenu complet
for(i in 1:nrow(unique_scales)) {
  # Obtenir les scores pour cette échelle
  scores <- scales_list$score_type_label[scales_list$scale == unique_scales$scale[i]]
  
  # Générer le texte complet avec description et référence
  cat(sprintf("::: {.card title=\"%s\"}\n%s\n\nScores mesurés :\n\n%s\n\nDescription :\n\n%s\n\nRéférence :\n\n%s\n:::\n\n",
      unique_scales$scale[i],
      unique_scales$scale_label[i],
      paste("- ", scores, collapse = "\n"),
      unique_scales$scale_description[i],
      unique_scales$scale_reference[i]))
}
...
```{r}
value_box(
  title = "Sous-groupe de référence",
  value = textOutput("SubgroupRef"),
  showcase = bs_icon("house")
)
```
...
output$scale_plot <- renderPlotly({
  ...
  reference_segments <- plot_data |> 
    ...
    summarise(
      subgroup_reference = first(subgroup_reference),
  ...
  p <- ggplot() +
    geom_segment(
      ...
      aes(x = start, xend = end,
          y = subgroup_reference, yend = subgroup_reference,
...
output$scale_table <- renderDT({
  ...
  table_data |> 
    rename(
      ...
      "Moyenne du sous-groupe" = subgroup_reference,
      "Écart-type du sous-groupe" = subgroup_sd,
      "N du sous-groupe" = subgroup_n,
      "Moyenne du groupe" = group_reference,
      "Écart-type du groupe" = group_sd,
      "Quartile" = quartile,
      "N du groupe" = group_n
      ) |>
    arrange(desc(`Date`)) |>
    ...
    formatRound(
      columns = c("Score personnel", "Moyenne du sous-groupe", "Écart-type du sous-groupe", "Moyenne du groupe", "Écart-type du groupe"),
      digits = 2
      ) |>
    formatDate(
      columns = "Date",
