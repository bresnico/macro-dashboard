...
output$group_status <- renderUI({
  req(input$group_id)
  valid_id <- any(data$group_id == input$group_id, na.rm = TRUE)
...
group_data <- reactive({
  ...
  # Valider que l'ID existe dans les données
  validate(need(
    any(data$group_id == input$group_id, na.rm = TRUE),
    ""
  ))
...
output$researcher_status <- renderUI({
 ...
 valid_code <- any(data$researcher_id == input$researcher_id, na.rm = TRUE)
 
 if(valid_code) {
   tagList(
     div(style = "color: green;", icon("check-circle"), "Code valide"),
     
     checkboxGroupInput("research_scales", "Échelles", 
       choices = unique(data$scale_label),
       selected = unique(data$scale_label)),
     
     checkboxGroupInput("research_groups", "Groupes",
       choices = unique(data$group_id),
       selected = unique(data$group_id)),
     
     checkboxGroupInput("research_status", "Statut",
       choices = unique(data$status),
       selected = unique(data$status)),
       
     checkboxGroupInput("research_gender", "Genre",
       choices = unique(data$gender),
       selected = unique(data$gender)),
       
     checkboxGroupInput("research_age", "Âge",
       choices = unique(data$age),
       selected = unique(data$age)),
       
     checkboxGroupInput("research_experience", "Expérience",
       choices = unique(data$experience),
       selected = unique(data$experience))
   )
 } else {
...
filtered_research_data <- reactive({
  req(input$researcher_id, input$research_scale, input$research_groups)
...
output$research_plot <- renderPlotly({
  if (is.null(input$researcher_id) || 
     nchar(input$researcher_id) == 0 || 
     !any(data$researcher_id == input$researcher_id, na.rm = TRUE)) return(NULL)
  
  req(input$researcher_id,
      input$research_scales,
  ...
  p <- data |>
    filter(
      ...
      experience %in% input$research_experience
      ) |>
...
output$download_button <- renderUI({
 req(input$researcher_id, any(data$researcher_id == input$researcher_id, na.rm = TRUE))
 downloadButton("downloadData", "Télécharger les données filtrées")
})

output$downloadData <- downloadHandler(
 filename = function() {
   paste("research_data_", Sys.Date(), ".csv", sep = "")
 },
